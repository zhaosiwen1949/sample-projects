...
workspace "atmospheric_sky_component"
...
filter "platforms:Win64"
    ...
    architecture "x64"
    postbuildcommands {
        '{MKDIR} %TM_SDK_DIR%/bin/data/shaders/atmospheric_sky_component', 
        '{COPY} ../../shaders/*.shader %TM_SDK_DIR%/bin/data/shaders/atmospheric_sky_component',
    }
    prebuildcommands {
...
project "atmospheric_sky_component"
    location "build/atmospheric_sky_component"
    targetname "tm_atmospheric_sky_component"
    kind "SharedLib"
    language "C++"
    files {"*.inl", "*.h", "*.c"}
    sysincludedirs { "" }
